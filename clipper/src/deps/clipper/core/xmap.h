...
private:
  ...
  */
  ...
  public:
    ...
    //! copy constructor
    Xmap(const Xmap<T>& other)
        {
            init( other.spacegroup(), other.cell(), other.grid_sampling() );
            Xmap_base::Map_reference_index ix;
            for (ix=first(); !ix.last(); ix.next()) {
                list[ix.index()] = other[ix];
            }
        }
  ...
  template<class T> void Xmap<T>::init(const Spacegroup& spacegroup, const Cell& cell, const Grid_sampling& grid_sam)
  {
