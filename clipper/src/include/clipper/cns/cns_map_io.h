...
#ifndef CLIPPER_CNS_MAP_IO
#define CLIPPER_CNS_MAP_IO
...
namespace clipper
{
  ...
  /*! This is the import/export class which can be linked to a CNS map
    file and be used to transfer data into or out of a Clipper
    data structure. */
  class CLIPPER_IMEX CNSMAPfile
  {
   public:
    ...
    //! import data to Xmap
    template<class T> void import_xmap( Xmap<T>& xmap );
    //! export data from Xmap
    template<class T> void export_xmap( const Xmap<T>& xmap );
    //! import data to NXmap
    template<class T> void import_nxmap( NXmap<T>& nxmap );
    ...
    template<class T> ASUerror import_xmap_check_asu( Xmap<T>& xmap, T missing );
