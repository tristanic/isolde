...
private:
    ...
    double _interpolate(const size_t &n, const std::vector<double> &corners, const size_t &i,
        const std::vector<std::pair<double, double>> &offsets);
    double _interpolate1d(const std::pair<double, double> &offset, const double& lower, const double& upper);
    void corner_offsets();
