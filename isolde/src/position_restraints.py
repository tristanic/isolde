...
class Atom_Position_Restraints(Position_Restraints):
    ...
    '''
    def __init__(self, session, master_model, atoms_or_list, triggers = None, include_hydrogens = False, create_target = False):
        '''
        ...
        if triggers is not None:
            if self.target_indicators is not None:
                self._add_restraint_handler = triggers.add_handler(
                    'position restraint added', self._update_active_list)
                self._remove_restraint_handler = triggers.add_handler(
    ...
    def _remove_handlers(self):
        ...
        if self._master_delete_handler is not None:
            ...
            self._master_delete_handler = None
        if self._add_restraint_handler is not None:
            self._triggers.remove_handler(self._add_restraint_handler)
            self._add_restraint_handler = None
        if self._remove_restraint_handler is not None:
            self._triggers.remove_handler(self._remove_restraint_handler)
            self._remove_restraint_handler = None
