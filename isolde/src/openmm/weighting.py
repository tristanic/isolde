...
def guess_mdff_weight(mdff_mgr, radius_multiplier = 0.15, constant = 1):
    '''
    ...
    grid_points = volume.grid_points(volume.data.ijk_to_xyz_transform)
    data = numpy.array(volume.data.matrix(), order='C')
    close_i = find_close_points(model.atoms.coords, grid_points, 3)[1]
    close_grid_vals = numpy.array(data.ravel()[close_i], order='C')
    from chimerax.map.data import Matrix_Value_Statistics
    ms = Matrix_Value_Statistics(close_grid_vals)
    total_vol = voxel_vol*len(close_grid_vals)
    r = 1 - model_vol*(radius_multiplier**3)/total_vol
    #return r
